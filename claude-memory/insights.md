...
12. **Prompt caching needs an explicit safe-composition boundary.** If provider-side caching is enabled on `system_prompt`, create a dedicated helper that accepts only stable instruction/schema text and treat it as the guardrail. Story text, ACs, comments, domain context, generated cases, and review feedback must stay out of cache-eligible system blocks.
